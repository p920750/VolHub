...
class _VolunteerProfilePageState extends State<VolunteerProfilePage> {
  ...
  Future<void> _pickProfileImage() async {
    ...
    if (picked != null) {
      ...
      // AFTER uploading image & getting avatarUrl
      await SupabaseService.updateUsersTable({'profile_photo_url': avatarUrl});
  ...
  void _calculateProfileCompletion() {
    ...
    if (_profileImage != null || (avatarUrl != null && avatarUrl!.isNotEmpty))
      completed++;
    if (email.isNotEmpty) completed++;
    if (fullName.isNotEmpty) completed++;
    if (phone.isNotEmpty) completed++;
  ...
  void _validateProfile() {
    setState(() {
      ...
      profileImageError =
          (_profileImage == null && (avatarUrl == null || avatarUrl!.isEmpty))
          ? 'Profile photo is required'
          : null;
    });
  }
  ...
  Future<void> _loadUserFromUsersTable() async {
    final data = await SupabaseService.getUserFromUsersTable();
    if (data == null) return;
  ...
  void initState() {
    ...
    _phoneFocus.addListener(() async {
      if (!_phoneFocus.hasFocus && editPhone) {
        ...
        await SupabaseService.updateUsersTable({'phone_number': phone});
