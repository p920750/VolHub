...
class _ProposalDetailsScreenState extends State<ProposalDetailsScreen> {
  ...
  Future<void> _loadRecommendedVolunteers() async {
    ...
    try {
      final response = await EventManagerService.getRecommendedVolunteers(widget.event['id'].toString());
      if (mounted) {
        setState(() {
          _recommendedVolunteers = response;
            ...
            if (widget.event['assigned_manager_id'] == EventManagerService.client.auth.currentUser?.id) ...[
              ...
              if (_isLoadingRecommendations)
                ...
                SizedBox(
                  ...
                  child: ListView.separated(
                    scrollDirection: Axis.horizontal,
                    itemCount: _recommendedVolunteers.take(5).length,
